...
def genetic_algorithm(product_data, population_size, num_generations, crossover_rate, mutation_rate):
    ...
    for _ in range(population_size):
        ...
        init_R_min, init_s_min, init_S_min = find_rss(to, product_sim)
        init_R = round(init_R_min)
        init_s = round(init_s_min)
        init_S = round(init_S_min)

        population.append((product_sim, demand_result, total_cost, to, init_R, init_s, init_S))

    # Capture the first individual
    first_product_sim, first_demand_result, total_cost, to, init_R, init_s, init_S = population[0]
    ...
    for generation in range(1, num_generations):
        ...
        for individual in population:
            product_sim, demand_result, total_cost, to, init_R, init_s, init_S = individual
            ...
            if total_demand == 0:
                total_demand = 1  # Prevent zero division
            fitness_score = 0.5 * total_cost + 0.5 * (unsold_orders / total_demand)
    ...
    best_solution = min(population, key=lambda x: min_fitness(*x)[0])
    # c_total, best_R, best_s, best_S, best_total_cost, best_to, best_demand, best_product, inventory_level_list, tot_dmd, tot_lost, max_inventory, purchases_freq, purchases_total, restock_data = best_solution
    best_product, best_demand, best_total_cost, best_to, best_R, best_s, best_S = best_solution
    ...
    inventory_level_list, tot_dmd, tot_lost, max_inventory, purchases_freq, purchases_total, restock_data = calculate_inventory_levels_rss(half_demand, best_R, best_s, best_S)
    ...
    return total_biaya_penyimpanan_list, to_penyimpanan_list, data_list, demand_result_list, orders_lost_list, inventory_level_list, tot_dmd, tot_lost, max_inventory, purchases_freq, purchases_total, restock_data, best_product, best_demand, best_total_cost, best_to, best_R, best_s, best_S, first_R, first_s, first_S, first_purchases_freq, first_tot_lost, first_demand_result, first_purchases_total
...
def min_fitness(product, demand, total_cost, to, init_R, init_s, init_S):
    # FIRST DATA
    ...
    inventory_level_list, tot_dmd, tot_lost, max_inventory, purchases_freq, purchases_total, restock_data = calculate_inventory_levels_rss(half_demand, init_R, init_s, init_S)

    total_demand = sum(tot_dmd)

    if std_dev_daily_demand == 0:
        std_dev_daily_demand = 1e-6

    # Cost Calculation
    if purchases_freq * init_R == 0:
        purchases_freq = 1
    init_R = 1  # Prevent zero division
    c_order = 35000 * (180 / (purchases_freq * init_R))
    c_hold = product["biaya_simpan"] * round((init_S + init_s) / 2) + round((total_demand * init_R) / purchases_freq)
    ...
    return c_total, product, demand, total_cost, to, init_R, init_s, init_S
...
def periodic_view(request):
    if request.method == 'POST':
        ...
        for index, x in enumerate(array):
            ...
            tp_list, to_list, data_list, demand_result_list, orders_lost_list, inventory_level_list, total_demand, total_lost, max_inventory, purchases_freq, purchases_total, restock_data, best_product, best_demand, best_total_cost, best_to, best_R, best_s, best_S, first_R, first_s, first_S, first_purchases_freq, first_total_lost, first_demand, first_purchases_total = genetic_algorithm(product, pop_size, num_generations, crossover_rate, mutation_rate)
...
def inventory_collab_view(request):
    if request.method == 'POST':
        ...
        if request.user.employee.role == 'superadmin':
                ...
                try:
                    ...
                    for item in items:
                        ...
                        try:
                            ...
                            best_total_cost, best_to, best_R, best_s, best_S, first_R, first_s, first_S, first_purchases_freq, first_total_lost, first_demand, first_purchases_total) = genetic_algorithm(
                    ...
                    data_all.append({'outlet': outlet, 'data': data, 'restock_plot': outlet_restock_plot})
            ...
            # Render the context
            context = {'data_all': data_all, 'total_data': total_data, 'outlet_inventory_levels': outlet_inventory_levels}
            ...
            for index, x in enumerate(array):
                ...
                tp_list, to_list, data_list, demand_result_list, orders_lost_list, inventory_level_list, total_demand, total_lost, max_inventory, purchases_freq, purchases_total, best_product, best_demand, best_total_cost, best_to, best_R, best_s, best_S, first_R, first_s, first_S, first_purchases_freq, first_total_lost, first_demand, first_purchases_total = genetic_algorithm(product, pop_size, num_generations, crossover_rate, mutation_rate)
