...
def log_scaled_mutation(individual, mutation_rate, sigma=0.1, lower_bound=1, upper_bound=10000):
    ...
    _, _, temp_R, temp_s, temp_S = mutated_individual
...
def genetic_algorithm(product_data, population_size, num_generations, crossover_rate, mutation_rate, daily_sales):
    ...
    first_total_cost, first_to = min_fitness(first_product, first_demand, first_R, first_s, first_S)
    ...
    for generation in range(num_generations):
        ...
        for individual in population:
            product_sim, demand_result, init_R, init_s, init_S = individual
            total_cost, total_stockout = min_fitness(product_sim, demand_result, init_R, init_s, init_S)
        ...
        for parent1, parent2 in parents:
            ...
            def fix_S_s(individual):
                prod, demand, R, s, S = individual
                S = max(s + 1, S)
                return (prod, demand, R, s, S)
    ...
    best_product, best_demand, best_R, best_s, best_S = best_solution
    best_total_cost, best_to = min_fitness(best_product, best_demand, best_R, best_s, best_S)
    ...
    return (total_biaya_penyimpanan_list, to_penyimpanan_list, data_list, demand_result_list, orders_lost_list, 
            inventory_level_list, tot_dmd, tot_lost, max_inventory, purchases_freq, purchases_total, restock_data,
...
# Periodic Review Function
def min_fitness(product, demand, init_R, init_s, init_S):
        ...
        if request.user.employee.role == 'superadmin':
            ...
            for outlet in outlets:
                ...
                for item in items:
                    ...
                    # temp_demand, temp_R, temp_s, temp_S = genetic_algorithm(product, pop_size, num_generations, crossover_rate, mutation_rate, daily_sales)
